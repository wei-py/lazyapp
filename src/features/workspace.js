...
export function identifyDocument(path) {
  ...
  if (
    parts[0] === 'platforms'
    && platformKinds.includes(parts[1])
    && parts.at(-1) === 'config.json'
  ) {
    if (parts.length === 3)
  ...
  if (
    parts.length === 2
    && Object.hasOwn(kinds, parts[0])
    && parts[1].endsWith('.json')
    && isSafeName(parts[1].slice(0, -5))
  ) {
  ...
  for (const examplePath of paths.filter(path => path.endsWith('.example'))) {
    ...
    if (!entries.has(path)) {
      entries.set(path, {
        ...
        missing: true,
        examplePath,
        instructionOnly: path.split('/').at(-1) === 'credentials',
      })
    }
  }
...
export async function saveDocument(session, path, kind, draft, revision) {
  ...
  if (!identified || identified.kind !== kind) {
    throw validationError([
      { key: '', message: 'Document kind does not match a supported configuration path' },
    ])
  }
...
export async function runDoctor(session, language = 'en') {
  ...
  const enabled = Array.isArray(app?.platforms)
    ? app.platforms.filter(kind => platformKinds.includes(kind))
    : []
  ...
  for (const kind of enabled) {
    ...
    if (
      ...
      && ![...documents.values()].some(
        document => document.kind === kind && document.scope === 'signing',
      )
    ) {
      report(
        'missing',
  ...
  for (const [path, document] of documents) {
    ...
    for (const issue of issues) {
      ...
      if (!disabled || !missing) {
        report(
          ...
          issue.messageKey || issue.message,
          path,
          issue.params,
        )
      }
    ...
    for (const field of documentFields(kind, { scope })) {
      ...
      if (
        ...
        || !reference
        || issues.some(issue => issue.key === field.key)
      ) {
        continue
      }
      try {
        ...
        if (!info.exists) {
          report(disabled ? 'warning' : 'missing', '{label}: referenced file is missing', path, {
            label: field.label,
          })
        }
        ...
        if (info.permissionsWarning) {
          report('warning', '{label}: file permissions allow access by other users', path, {
            label: field.label,
          })
        }
      }
    ...
    if (
      documentFields(kind, { scope }).some(
        field =>
          field.type === 'secret'
  ...
  try {
    for (const warning of await session.permissionWarnings())
      report('warning', warning.label, warning.path)
  }
  catch {

...
function components(relative) {
  if (typeof relative !== 'string' || !relative || path.isAbsolute(relative) || /[\\:\x00-\x1F\x7F]/u.test(relative))
    throw failure('PATH_ESCAPE', 'Use a nonempty workspace-relative path without traversal or separators from another operating system.')
  const parts = relative.split('/')
  if (parts.some(part => !part || part === '.' || part === '..' || part.startsWith('.')))
    throw failure('PATH_ESCAPE', 'Hidden, empty, and traversal path components are not managed file paths.')
  return parts
}

function assertDocument(data, relative) {
  if (!data || typeof data !== 'object' || Array.isArray(data))
    throw failure('INVALID_DOCUMENT', `Expected a JSON object at ${relative}; reload or repair it before saving.`)
  if (data.schemaVersion !== 1)
    throw failure('UNSUPPORTED_SCHEMA', `Unsupported schemaVersion at ${relative}; this version supports schemaVersion 1 only. No migration or overwrite was performed.`)
}
...
function parseDocument(bytes, relative) {
  ...
  try {
    ...
    throw failure('INVALID_JSON', `Invalid JSON at ${relative}; repair the file or restore a backup before saving.`)
  }
...
async function readRegular(target) {
  ...
  try {
    if (!(await handle.stat()).isFile())
      throw failure('INVALID_FILE', 'The target must be a regular file, not a directory or special device.')
...
export async function inspectExternalReference(reference) {
  if (typeof reference !== 'string' || !path.isAbsolute(reference) || /[\x00-\x1F\x7F]/u.test(reference))
  ...
  try {
    ...
    try {
      if (!(await handle.stat()).isFile())
        throw failure('INVALID_FILE', 'An external reference must point to a readable regular file.')
    }
    finally {
      await handle.close()
    }
    return { path: reference, external: true, exists: true, regular: true, permissionsWarning: false }
  }
  catch (error) {
    if (error.code === 'ENOENT')
      return { path: reference, external: true, exists: false, regular: false, permissionsWarning: false }
    throw error
  }
}
...
async function acquireLock(root) {
  ...
  try {
    ...
    try {
      ...
      if (Number.isSafeInteger(raw.pid) && typeof raw.hostname === 'string' && typeof raw.startedAt === 'string')
    ...
    const locked = failure('WORKSPACE_LOCKED', `Workspace is locked at ${target}. Close its owning instance. If it terminated, verify the owner is no longer running before manually removing this lock; locks are never recovered automatically.`)
...
async function createSession(root, owner) {
  ...
  async function checkRoot() {
    ...
    if (!current || !current.isDirectory() || current.isSymbolicLink() || current.dev !== identity.dev || current.ino !== identity.ino)
      throw failure('PATH_ESCAPE', 'Workspace directory changed or became a symlink. Close and reopen the workspace.')
  }
  ...
  async function targetPath(relative, createParents = false) {
    ...
    for (let index = 0; index < parts.length; index++) {
      ...
      if (entry?.isSymbolicLink())
        throw failure('PATH_ESCAPE', `Symlink paths are not managed: ${relative}. Use an explicit external reference instead.`)
      ...
      if (entry && parent && await fs.realpath(target) !== target)
        throw failure('PATH_ESCAPE', `A parent directory resolves outside its expected location: ${relative}.`)
    }
    return target
  }
  ...
  function ensureOpen() {
    if (closing)
      throw failure('SESSION_CLOSED', 'Workspace session is closing or closed; reopen before accessing it.')
  }
  ...
  async function ensureLock() {
    ...
    try {
      ...
      throw failure('WORKSPACE_LOCKED', 'Workspace lock was removed or changed; close and reopen before writing.')
    }
    if (current.token !== owner.token)
      throw failure('WORKSPACE_LOCKED', 'Workspace lock ownership changed; close and reopen before writing.')
  }
    ...
    try {
      ...
      if (overwrite) {
        ...
        try {
          ...
          if (error.code === 'EEXIST')
            throw failure('FILE_EXISTS', `A file already exists at ${relative}; confirm overwrite or choose a different name.`)
          throw error
        }
      }
    }
  ...
  async function walk(directory, prefix, visit) {
    for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
      ...
      if (info.isSymbolicLink())
        throw failure('PATH_ESCAPE', `Workspace contains a symlink at ${relative}; remove it or use an explicit external reference.`)
  ...
  return {
      ...
      return serialize(async () => {
        ...
        const check = async () => {
          ...
          if (current.revision !== expectedRevision)
            throw failure('REVISION_CONFLICT', `The document changed on disk at ${relative}; reload before saving. No data was overwritten.`)
        }
        await check()
        await replace(relative, handle => handle.writeFile(encoded), check, original.revision !== null)
    ...
    async importFile(source, destination, { overwrite = false } = {}) {
      return serialize(async () => {
        if (destination === 'app.json')
          throw failure('INVALID_FILE', 'App metadata must be saved as a versioned document, not replaced through file import.')
        const absoluteSource = path.resolve(source)
        const sourceHandle = await fs.open(absoluteSource, constants.O_RDONLY | constants.O_NONBLOCK)
        try {
          ...
          if (existing && sourceInfo.dev === existing.dev && sourceInfo.ino === existing.ino)
            throw failure('INVALID_FILE', 'Source and destination refer to the same file; choose a different destination.')
          if (existing && !overwrite)
            throw failure('FILE_EXISTS', `A file already exists at ${destination}; explicit overwrite confirmation is required.`)
          const check = async () => {
            const current = await statOrNull(await targetPath(destination))
            if (!overwrite && current)
              throw failure('FILE_EXISTS', `A file already exists at ${destination}; explicit overwrite confirmation is required.`)
          }
          await replace(destination, async (handle) => {
            ...
            while (true) {
              ...
              while (offset < bytesRead) {
                const { bytesWritten } = await handle.write(buffer, offset, bytesRead - offset, null)
                if (bytesWritten === 0)
                  throw failure('EIO', 'Import could not write file contents; the original destination was not changed.')
                offset += bytesWritten
              }
            }
          }, check, overwrite && Boolean(existing))
          return destination
        }
        finally {
    ...
    async removeFile(relative) {
      return serialize(async () => {
        if (relative === 'app.json')
          throw failure('INVALID_FILE', 'The workspace app.json cannot be deleted through file management.')
        const target = await targetPath(relative)
        const current = await statOrNull(target)
        if (!current)
          throw Object.assign(new Error(`File not found: ${relative}`), { code: 'ENOENT' })
        if (!current.isFile() || current.isSymbolicLink())
          throw failure('INVALID_FILE', 'Only explicitly selected internal regular files can be deleted.')
        await fs.unlink(target)
      })
    },
    async inspectReference(reference) {
      ...
      return { path: reference, external: false, exists: Boolean(entry), regular: Boolean(entry?.isFile()), permissionsWarning: Boolean(entry && process.platform !== 'win32' && (entry.mode & 0o077)) }
    },
    async permissionWarnings() {
      ...
      const inspect = async (relative, info) => {
        if (info.mode & 0o077)
          warnings.push({ path: relative, label: 'Permissions allow group or other users access; review permissions manually.' })
      }
...
export async function openWorkspace(projectDir) {
  ...
  if (!info)
    throw failure('WORKSPACE_MISSING', `No workspace exists at ${root}. Initialize only after confirmation.`)
  if (!info.isDirectory() || info.isSymbolicLink())
    throw failure('WORKSPACE_CONFLICT', `The path ${root} already exists and is not a regular workspace directory. Choose another project or move the conflicting path.`)
  const appPath = path.join(root, 'app.json')
  const appInfo = await statOrNull(appPath)
  if (!appInfo || !appInfo.isFile() || appInfo.isSymbolicLink())
    throw failure('WORKSPACE_CONFLICT', `The existing directory ${root} has no regular app.json; it will not be adopted or overwritten.`)
...
export async function initializeWorkspace(projectDir, app, { documents = [], imports = [], examples = [] } = {}) {
  ...
  try {
    ...
    try {
      ...
      if (error.code === 'EEXIST')
        throw failure('WORKSPACE_CONFLICT', `The path ${root} appeared during initialization; nothing was overwritten.`)
      throw error
    }
    try {

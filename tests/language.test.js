...
const APP = { schemaVersion: 1, name: 'English 中文', description: 'Cancel', platforms: [], environments: [], version: '1.0.0' }
...
function application(preferences) {
  ...
  const app = new Application(join(root, 'project'), () => {}, () => {}, {
    preferences: preferences || { load: () => loadPreferences(options), save: value => savePreferences(value, options) },
  })
  app.state.documents = [{ path: 'app.json', kind: 'app', data: structuredClone(APP), revision: 'original' }]
  apps.push(app)
  return app
}
...
describe('language settings interactions', () => {
  ...
  test('missing workspace reports a localized error without an initialization dialog or writes', async () => {
    ...
    expect(t('en', 'No lazyapp workspace found. Create one? (y/N): ')).toBe('No lazyapp workspace found. Create one? (y/N): ')
  ...
  test('Doctor labels can change language without another read or leaking credential values', async () => {
    let reads = 0
    const documents = { 'app.json': APP, 'services/push.json': { schemaVersion: 1, name: 'Cancel', token: 'fictional-private-token' } }
  ...
  test('template parameters remain literal and unknown exception text never becomes translated UI', () => {
    expect(t('zh', 'Path: {path}', { path: 'services/Cancel-{label}.json' })).toBe('路径：services/Cancel-{label}.json')

...
describe('workspace persistence and initialization', () => {
  test('discovery and canceled initialization never create files or scan parents', async () => {
    ...
    await expect(openWorkspace(path.join(project, 'nested'))).rejects.toMatchObject({
      ...
      documents: [
        {
          ...
          data: { schemaVersion: 1, certificate: 'platforms/ios/preview/certificate.p12' },
        },
      ],
  ...
  test('failed staged imports leave no workspace or staging debris', async () => {
    await expect(
      initializeWorkspace(project, APP, {
        documents: [
          { path: 'services/example.json', data: { schemaVersion: 1, token: 'fictional-token' } },
        ],
  ...
  test('existing directories and duplicate staged destinations are never overwritten', async () => {
    ...
    await expect(initializeWorkspace(project, APP)).rejects.toMatchObject({
  ...
  test('corrupt JSON and unsupported versions cannot be overwritten or treated as missing', async () => {
    ...
    await expect(session.save('app.json', APP, initial.revision)).rejects.toMatchObject({
      code: 'INVALID_JSON',
    })
    ...
    await expect(session.save('other.json', { schemaVersion: 2 }, null)).rejects.toMatchObject({
      code: 'UNSUPPORTED_SCHEMA',
    })
  ...
  test('new directories, files, locks, and imported credentials have private POSIX permissions', async () => {
    ...
    const session = await initialize({
      imports: [{ source, destination: 'assets/keys/file.key' }],
    })
    for (const directory of [
      session.root,
      path.join(session.root, 'assets'),
      path.join(session.root, 'assets/keys'),
    ])
...
describe('serialized commits, locks, and external edits', () => {
  test('refuses another instance with owner information and refuses stale locks without removing them', async () => {
    ...
    await expect(openWorkspace(project)).rejects.toMatchObject({
  ...
  test('external edits conflict without changing originals and queued writes compare revisions at commit', async () => {
    ...
    await expect(
      session.save('app.json', { ...APP, name: 'Draft' }, initial.revision),
    ...
    expect(
      (await fs.readdir(session.root)).filter(name => name.startsWith('.lazyapp-tmp-')),
    ).toEqual([])
  ...
  test('serialization errors leave originals unchanged and close drains pending writes', async () => {
    ...
    const cyclic = { ...APP }
    cyclic.self = cyclic
    await expect(session.save('app.json', cyclic, snapshot.revision)).rejects.toMatchObject({
...
describe('file boundaries and external read-only references', () => {
  test('rejects traversal, absolute destinations, hidden paths, and symlink parents and leaves external files unchanged', async () => {
    ...
    for (const relative of [
      ...
      await expect(session.save(relative, { schemaVersion: 1 }, null)).rejects.toMatchObject({
        code: 'PATH_ESCAPE',
      })
    ...
    await expect(session.importFile(external, 'linked/other.key')).rejects.toMatchObject({
      code: 'PATH_ESCAPE',
    })
    await expect(session.removeFile('linked/outside.key')).rejects.toMatchObject({
      code: 'PATH_ESCAPE',
    })
    ...
    await expect(session.inspectReference('file.key')).rejects.toMatchObject({
      code: 'PATH_ESCAPE',
    })
    await expect(
      session.importFile(external, 'file.key', { overwrite: true }),
  ...
  test('copy import requires explicit overwrite, preserves source, and deletes only internal regular files', async () => {
    ...
    expect(await inspectExternalReference(source)).toMatchObject({
      external: true,
      exists: true,
      regular: true,
    })
    expect(await session.inspectReference(source)).toMatchObject({
      external: true,
      exists: true,
      regular: true,
    })
    expect(await session.inspectReference('assets/missing.key')).toMatchObject({
      external: false,
      exists: false,
    })
    expect(await inspectExternalReference(path.join(project, 'missing'))).toMatchObject({
      exists: false,
    })
    await expect(inspectExternalReference('relative.key')).rejects.toMatchObject({
      code: 'PATH_ESCAPE',
    })
    ...
    await expect(session.importFile(source, 'assets/key.key')).rejects.toMatchObject({
      code: 'FILE_EXISTS',
    })
    ...
    expect(await fs.readFile(path.join(session.root, 'assets/key.key'), 'utf8')).toBe(
      'updated source',
    )
    await expect(
      session.importFile(path.join(session.root, 'assets/key.key'), 'assets/key.key', {
        overwrite: true,
      }),
